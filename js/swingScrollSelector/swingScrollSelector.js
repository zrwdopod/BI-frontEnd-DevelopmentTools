...
(function ($) {
    //实例化入口
    $.fn.swingScrollSelector = function () {
        ...
        if (methods[method]) {
            ...
            $.error('jQuery.swingScrollSelector中不存在方法' + method);
            return this;
        }
        ...
        init: function (options, data) {
            ...
            return this.each(function () {
                ...
                if (typeof (settings) == 'undefined') {
                    var defaultSettings = swingScrollSelectorDefault;
                ...
                var simpleData = [{ "name": "APP1", "配置ID": "1", "items": [{ "name": "ROLE1-1", "配置ID": "2" }, { "name": "ROLE1-2", "配置ID": "3" }] }, { "name": "APP2", "配置ID": "5", "items": [{ "name": "ROLE2-1", "配置ID": "6" }, { "name": "ROLE2-2", "配置ID": "7" }, { "name": "ROLE2-3", "配置ID": "8" }] }];
                var complexData = [{ "ID": "01", "name": "默认车系", "items": [{ "ID": "0101", "name": "默认车型", "swingScrollSelector": [{ "name": "APP1", "配置ID": "1", "items": [{ "name": "ROLE1-1", "配置ID": "2" }, { "name": "ROLE1-2", "配置ID": "3" }] }, { "name": "APP2", "配置ID": "5", "items": [{ "name": "ROLE2-1", "配置ID": "6" }, { "name": "ROLE2-2", "配置ID": "7" }, { "name": "ROLE2-3", "配置ID": "8" }] }] }] }];
                ...
                // 执行代码
                $this.empty();
                if (!$this.hasClass('swingScrollSelector')) {
                    $this.addClass('swingScrollSelector');
                }
                ...
                if (settings.LogConfig.level >= 3 && settings.LogConfig.isLogInfo) {
                    Diablo.printLog('界面元素', false, '实例化', '实例化swingScrollSelector');
                }
    ...
    function _bind(panel, myBindData, mySettings) {
        ...
        $panel.find('.swingTreelist').val('').scroller('destroy').scroller({ "preset": "list", "labels": ["程序集", "权限"], "theme": "sense-ui", "mode": "scroller", "display": "inline", "lang": "" });
    ...
    function _getGroupDiv(parent, myBindData, mySettings) {
        ...
        for (var i in myBindData) {
            ...
            else if (myBindData[i].swingScrollSelector) {
                if (myBindData[i].swingScrollSelector.length > 0) {
                    $h3.click(function () {
                        $this = $(this);
                        if ($this.parent().hasClass('ui-collapsible-collapsed')) {
                            _explandNode(this);
                    ...
                    $content.append(_createContent(myBindData[i].swingScrollSelector[0].inputs, mySettings));
                }
            }
            else {
    ...
    function _createContent(zData, mySettings) {
        ...
        var $dummyWrap = $('<div class="swingPanelWrap"></div>');
        $dummy = _diGui(zDataCopy);
        $dummy.addClass('swingTreelist');
        $dummyWrap.append($dummy);

        var $btnStart = $('<a href="#" data-role="button" data-icon="arrow-r" data-corners="true" data-shadow="true" data-iconshadow="true" data-wrapperels="span" data-theme="c" class="ui-btn ui-shadow ui-btn-corner-all ui-mini ui-btn-icon-left ui-btn-hover-' + mySettings.UI.PanelStyle.innerTheme + ' ui-btn-up-' + mySettings.UI.PanelStyle.innerTheme + ' ui-btn-inline"><span class="ui-btn-inner"><span class="ui-btn-text">启动</span><span class="ui-icon ui-icon-arrow-r ui-icon-shadow">&nbsp;</span></span></a>');
        $btnStart.bind('click', function () {

            var ddddd = $('.swingTreelist').siblings('input:eq(0)').val();
            var a_list = mySettings.Events;


            for (var i in a_list) {
                if (a_list[i].type = 'btnStartClick') {
                    var Handler = {};
                    var zrw = '';
        ...
        var zrw = '';
        $dummyWrap.append($btnStart);

        return $dummyWrap;
    }

    function _diGui(oData) {
        var $ul = $('<ul></ul>');
        for (var i in oData) {
            var $li = $('<li>' + oData[i].名称 + '</li>');
            $ul.append($li);
            if (oData[i].items) {
                var zrw = '';
                $li.append(arguments.callee(oData[i].items));
            }
        }
        return $ul;
    }
